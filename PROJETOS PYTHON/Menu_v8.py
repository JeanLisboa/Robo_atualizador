...
class Feedback:
    ...
    def __init__(self, master, opcao_mes):
        ...
        self.vendedor.config(textvariable=self.op_vend, values=('000671 MARLEI', '000508 JEANY', '000595 GUALTER', '000656 FLAVIO', '000641 LUCIA', '000502 CLEBER', '000654 PAULO BESERRA'))
    ...
    def report_comercial(self):  # envio das informações via wtsp (py autugui)
        a = self.op_vend.get()
        print(a)
        cod_vend = ''
        msg_vend = ''
        ...
        if a == '000641 LUCIA':
            ...
            msg_vend = str('Bom dia Lúcia, tudo bem ?')
            contato = ['+5583993637279']
        if a == '000502 CLEBER':
        ...
        # tel_vend = {'000671 MARLEI': '+558399637279',
        #             '000508 JEANY': '+558399637279',
        #             '000595 GUALTER': '+558399637279',
        #             '000656 FLAVIO': '+558399637279',
        #             '000641 LUCIA': '+558399637279',
        #             '000502 CLEBER': '+558399637279',
        #             '000654 PAULO BESERRA': '+558399637279'}
        #
        # nome_vend = {'000671 MARLEI': 'Marlei',
        #              '000508 JEANY': 'Jeany',
        #              '000595 GUALTER': 'Gualter',
        #              '000656 FLAVIO': 'Flávio',
        #              '000641 LUCIA': 'Lúcia',
        #              '000502 CLEBER': 'Cléber',
        #              '000654 PAULO BESERRA': 'Paulo'}
        # tel = tel_vend[a]
        # tel = str(tel)
        # print(tel)
        # nome = nome_vend[a]
        # nome = str(nome)
        # print(nome)
        # mensagem = (f'Bom dia{nome}! Seguem informações: ')

        df = pd.read_excel('G:/Drives compartilhados/CUSTOMER SERVICE/AREA 02 NE/Tracking BNE - 2023 - TESTE.xlsx',
                           sheet_name='BASE TRACKING 2023')  # tracking bne

        relatorio = df.loc[
            (df['STATUS'] != 'ENTREGUE') & (df['STATUS'] != 'DEVOLUÇÃO'), ["VEND", "DOC", "CLIENTE", "DESTINATÁRIO",
                                                                           "MUNICIPIO", "VOLUME", "Agend",
                                                                           "PREVISÃO ENTREGA", "OBS", "D+"]]
        rel = relatorio.loc[
            relatorio['Agend'] == 's', ["VEND", "DOC", "CLIENTE", "DESTINATÁRIO", "MUNICIPIO", "VOLUME", "Agend", "D+",
                                        "PREVISÃO ENTREGA", "OBS"]]

        relatorio2 = rel.loc[
            rel['VEND'] == cod_vend, ["D+", "Agend", "VEND", "DOC", "DESTINATÁRIO", "MUNICIPIO", "VOLUME",

                                      "PREVISÃO ENTREGA", "OBS"]]
        relatorio3 = \
            [
            ["D+", "DOC", "Agend", "DESTINATÁRIO", "MUNICIPIO", "VOLUME", "PREVISÃO ENTREGA", "OBS"]]
        ...
        for p in contato:
            pywhatkit.sendwhatmsg_instantly(phone_no=p, wait_time=13, tab_close=True, close_time=3,
                                            message=f'{msg_vend},\n Seguem informações: \n')

            for i in rel3.head(100).iterrows():
                pywhatkit.sendwhatmsg_instantly(phone_no=p, wait_time=13, tab_close=True, close_time=3,
                                                message=(
                                                    f"{i[1]['DOC']:<4} {i[1]['DEST']:.<33} {i[1]['OBS']:.>20}"f""))

                print(f"{i[1]['DOC']:<4} {i[1]['DEST']:.<33} {i[1]['OBS']:.>20}"f"")
        ...
        #                 pywhatkit.sendwhatmsg_instantly(phone_no=p, wait_time=13, tab_close=True, close_time=3,
        #                                                 message=(
        #                                                     f"{i[1]['DOC']:<4} {i[1]['DEST']:.<33} {i[1]['OBS']:.>20}"
        #                                                     f""))
        #
        #             print(p)
    ...
    def dataframe(self): # ainda sem uso...

        if self.dist == '1':
            print(f'{self.dist} - Alhandra Selecionada')
            self.df = pd.read_excel('G:\Drives compartilhados\CUSTOMER SERVICE\AREA 02 NE\Tracking BNE - 2023.xlsx',
                               sheet_name='BASE TRACKING 2023')
            print(self.df)
        if self.dist == '2':
            print(f'{self.dist} - Extrema Selecionada')
            self.df = pd.read_excel('G:\Drives compartilhados\CUSTOMER SERVICE\AREA 02 NE\TRACKING NE-BS-2023.xlsx',
                               sheet_name='BASE TRACKING 2023')
            print(self.df)
        return self.df
    ...
    def executar(self):
        ...
        if processo == '2':
            ...
            messagebox.showinfo('Processo Concluído', 'Datas de Agendamento Atualizadas')
        if processo == '3':
            Intranet.acessa_tela_atualizar_entrega(self)
            messagebox.showinfo('Processo Concluído', 'Datas de Entrega Atualizadas')
        if processo =='0':
