...
class Menu:
    ...
    op_processo = int(input("Atualizações (1) | Análises (2): "))

    if op_processo == 2:
        print('---Menu Análises---')
        op_analise = input('(1) Lista NFS não entregues: ')

    if op_processo == 1:

        print('Menu Atualizações')
        if dist == 2:
            processo = int(input("                              *** ATUALIZAÇÕES ***\n"
                                 "(1) Atualiza Expedição (Alhandra) | (2) Atualiza Agenda | (3) Atualiza Entrega \n"
                                 "Digite a Opção: "))
        else:
            processo = int(input("       *** ATUALIZAÇÕES ***\n"
...
class Tracking:
    def importa_nfs_nao_entregues(self):
        print('Class: Tracking, Método: importa_nfs_nao_entregues')
        pass
    def importa_data_expedicao(self):
        print('Class: Tracking, Método: importa_data_expedicao')
    ...
    def importa_data_entrega(self):
        ...
        relatorio = Menu.df.loc[Menu.df['STATUS'] == 'ENTREGUE', ["STATUS", "DOC", "DTA ENTREGA"]]
        relatorio = relatorio.dropna(axis=0)  # exclui as células  vazias
...
class Intranet:
    ...
    def acessa_tela_pedidos_otc(self):
        ...
        # acessar pedidos otc
        Intranet.navegador.find_element('xpath', '//*[@id="navigation"]/ul/li[2]/ul/a[3]').click()

        if Menu.dist == 1:  # extrema
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_DOWN)
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_DOWN)

        if Menu.dist == 2:  # Alhandra
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_UP)
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_UP)
    ...
    def acessa_tela_atualizar_expedicao(self):
        ...
        if Menu.dist == 3:  # seleciona a dist bs-alhandra
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_UP)
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_UP)
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_DOWN)
        if Menu.dist == 1:  # seleciona a dist bs-extrema
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_UP)
            Intranet.navegador.find_element('xpath', '/html/body/form[2]/table[1]/tbody/tr[2]/td[1]/select').send_keys(
                Keys.ARROW_UP)
        ...
        if Menu.dist == 1:  # número de série da NF
    ...
    def gera_lista_ped_sem_agenda(self):
        ...
        if Menu.dist == 1 and area in lst_area :
...
if Menu.op_analise == 1:
    Tracking.importa_nfs_nao_entregues(self=Tracking)
...
class Analise:
    ...
    pass
# USUÁRIO ACESSA ANALISE PARA VERIFICAR A OPÇÕES DO MENU
# OPÇÕES 1 - LISTAR NF'S AGENDADAS

#automação de fluxo de informação

# baruel X transportador

# envio de email informando:
    # entregas agendadas em d+2 e d+1
    # entregas não agendadas prazo expirando em d+2 e d+1

# Baruel x comercial
    # email / bot sobre nfs em transito para o comercial